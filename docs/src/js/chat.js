...
function makeHandler(msgs, typingEl, input, sendBtn) {
  ...
  let qStep = -1;
  const qAnswers = [];
  const history = []; // conversation history sent to backend for context
  ...
  return async function send() {
    ...
    if (IS_BACKEND_CONFIGURED) {
      try { reply = await backendChat(text, history); } catch { /* fall through */ }
    }
    ...
    if (realExchanges === 4 && qStep < 0) {
      ...
      setTimeout(() => {
        appendMsg(msgs, 'bot', fmt(
          `I'd love to help figure out if TaalumaMail fits your situation — mind if I ask a couple of quick questions? 😊\n\n${QUALIFY_Q[0]}`
        ));
        sendBtn.disabled = false; input.focus();
      }, 800);
      return;
...
export function initFloatChat({ panelId, btnId, inputId, sendBtnId, messagesId }) {
  ...
  function resetChat() {
    msgs.innerHTML = '';
    appendMsg(msgs, 'bot', GREETING_MSG);
    handler = makeHandler(msgs, null, input, sendBtn);
